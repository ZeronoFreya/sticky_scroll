...
function scrollbarDom(el, refEl, scrollDelta, scroll) {
    if (scroll.includes('x')) {
        ...
        const { track, thumb } = activeScrollBar(refEl, scrollDelta, 'x')
    ...
    if (scroll.includes('y')) {
        ...
        const { track, thumb } = activeScrollBar(refEl, scrollDelta, 'y')
                ...
                if (event.shiftKey) {
                    ...
                    if (scrollLeft <= 0) {
                        ...
                        if (event.wheelDeltaY > 0) {
                            // 上滚 → 想往左 → 继续拉伸（负）
                            scrollDelta.x -= event.wheelDeltaY
                            scrollDelta.x = Math.max(
                                scrollDelta.x,
                            ...
                            if (scrollDelta.x < 0) {
                                // 只有已经拉伸了才处理
                                event.preventDefault()
                                if (scrollDelta.x > event.wheelDeltaY) {
                                    // 吸附条件原样保留
                                    scrollDelta.x = 0
                                } else {
                                    scrollDelta.x -= event.wheelDeltaY // wheelDeltaY 为负
                        ...
                        if (event.wheelDeltaY > 0) {
                            ...
                            // 下滚 → 想往右 → 继续拉伸（正）
                            scrollDelta.x -= event.wheelDeltaY // wheelDeltaY 为负 → 变正
                            scrollDelta.x = Math.min(
                                scrollDelta.x,
                    ...
                    if (scrollTop <= 0) {
                        ...
                        if (event.wheelDeltaY > 0) {
                            // 上滚 → 继续往上拉（负）
                            scrollDelta.y -= event.wheelDeltaY
                            scrollDelta.y = Math.max(
                                scrollDelta.y,
                            ...
                            if (scrollDelta.y < 0) {
                                event.preventDefault()
                                if (scrollDelta.y > event.wheelDeltaY) {
                                    // 吸附
                                    scrollDelta.y = 0
                                } else {
                                    scrollDelta.y -= event.wheelDeltaY
                        ...
                        if (event.wheelDeltaY > 0) {
                            ...
                            // 下滚 → 继续往下拉（正）
                            scrollDelta.y -= event.wheelDeltaY // wheelDeltaY < 0 → 正值
                            scrollDelta.y = Math.min(
                                scrollDelta.y,
